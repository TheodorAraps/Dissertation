...
Με τα παραπάνω, ο \en{LoRa Basics Station} τρέχει ως \en{container} στο \en{Raspberry Pi} και 
...
% %%%%   Υποενότητα 5.2.2: Τροφοδοσία και επίπεδα λογικής   %%%%

% \subsection{Τροφοδοσία και επίπεδα λογικής}
...
%%%%   Υποενότητα 5.2.2: Βιβλιοθήκες και ρυθμίσεις IDE   %%%%
...
// PZEM Configuration
#define NUM_PZEMS 3
SoftwareSerial pzemSWSerial1(9, 2);  // RX, TX for PZEM 1
SoftwareSerial pzemSWSerial2(10, 3); // PZEM 2
SoftwareSerial pzemSWSerial3(11, 4); // PZEM 3
PZEM004Tv30 pzems[NUM_PZEMS];

// Energy reset configuration
unsigned long lastResetTime = 0;
const unsigned long resetInterval = 15 * 60000; // 15 minutes in milliseconds
...
void setup() {
  ...
  // Initialize PZEMs
  pzems[0] = PZEM004Tv30(pzemSWSerial1);
  pzems[1] = PZEM004Tv30(pzemSWSerial2);
  pzems[2] = PZEM004Tv30(pzemSWSerial3);
...
void loop() {
  ...
  if (millis() - lastResetTime >= resetInterval) {
    ...
    debugSerial.println("Energy counters reset");
  }

  // Buffer for LoRaWAN payload (36 bytes total - 12 bytes per PZEM)
  uint8_t data[NUM_PZEMS * 12];
  ...
  for (int i = 0; i < NUM_PZEMS; i++) {
    ...
    if (isnan(voltage) || isnan(current) || isnan(power) || 
      ...
      debugSerial.print("Error reading data from PZEM ");
    ...
    // Debug output
    debugSerial.print("PZEM "); debugSerial.print(i + 1); debugSerial.println(" Readings:");
    debugSerial.print("Voltage: "); debugSerial.print(voltage); debugSerial.println(" V");
    debugSerial.print("Current: "); debugSerial.print(current, 3); debugSerial.println(" A");
    debugSerial.print("Power: "); debugSerial.print(power); debugSerial.println(" W");
    debugSerial.print("Energy: "); debugSerial.print(energy, 4); debugSerial.println(" kWh");
    debugSerial.print("Frequency: "); debugSerial.print(frequency); debugSerial.println(" Hz");
    debugSerial.print("Power Factor: "); debugSerial.println(pf);
    debugSerial.println();

    // Prepare data for LoRaWAN (each value encoded as 2 bytes)
    int index = i * 12;
    data[index]      = (uint16_t(voltage * 10) >> 8);
    data[index + 1]  = uint16_t(voltage * 10) & 0xFF;
    data[index + 2]  = (uint16_t(current * 100) >> 8);
    data[index + 3]  = uint16_t(current * 100) & 0xFF;
    data[index + 4]  = (uint16_t(power * 10) >> 8);
    data[index + 5]  = uint16_t(power * 10) & 0xFF;
    data[index + 6]  = (uint16_t(energy * 100) >> 8);  
    data[index + 7]  = uint16_t(energy * 100) & 0xFF;
    data[index + 8]  = (uint16_t(frequency * 10) >> 8);
    data[index + 9]  = uint16_t(frequency * 10) & 0xFF;
    data[index + 10] = (uint16_t(pf * 100) >> 8);
    data[index + 11] = uint16_t(pf * 100) & 0xFF;
  }

  // Send data to TTN
  debugSerial.println("-- Sending data to TTN");
  if (send) ttn.sendBytes(data, sizeof(data));
...
void resetEnergyCounters() {
  for (int i = 0; i < NUM_PZEMS; i++) {
    pzems[i].resetEnergy();
    debugSerial.print("Reset energy for PZEM "); debugSerial.println(i+1);
  }
}  
\end{lstlisting}
\end{otherlanguage*}
...
\begin{itemize}
  \item \textbf{Βιβλιοθήκες:} \en{TheThingsNetwork}, \en{PZEM004Tv30}, \en{SoftwareSerial}.
  \item \textbf{\en{Board}:} Η αντίστοιχη επιλογή της πλακέτας \en{The Things Uno} (ή \en{Arduino Uno}/\en{Leonardo}, ανάλογα με την υλοποίηση).
  \item \textbf{\en{Port}/\en{Programmer}:} Σύμφωνα με το \en{USB} \en{serial} της πλακέτας.
  \item \textbf{\en{FreqPlan}:} \en{EU868}.
\end{itemize}
