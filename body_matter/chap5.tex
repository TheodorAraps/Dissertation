...
inline uint16_t encodeScaled(float v, float scale§\cyan{)}§ {
  // NaN or Inf or negative -> invalid
  if (!isfinite(v§\cyan{)}§ || isnan(v§\cyan{)}§ || v < 0.0§\cyan{f}§ || scale <= 0.0§\cyan{f)}§ return INVALID16;   
...
inline void putFloatBE(uint8_t *buf, int idx, float energy§\cyan{)}§ {
  // NaN or Inf or negative -> sentinel
  if (!isfinite(energy§\cyan{)}§ || isnan(v§\cyan{)}§ || v < 0.0§\cyan{f}§ || scale <= 0.0§\cyan{f)}§ {    
...
\begin{enumerate}
  ...
  \begin{Illustration}[!ht] 
    \centering      
    \includegraphics[width=1\textwidth]{figures/Choose_end_device_type.png}       
  ...
  \begin{Illustration}[!ht] 
    ...
    \label{figure5.23}    
  \end{Illustration} 
  \item Τέλος, πατάμε \textit{\en{Register end device}} και ολοκληρώνουμε την διαδικασία. Πλέον εμφανίζεται η συσκευή μας στο μενού \textit{\en{End devices}} της εφαρμογής:
  \begin{Illustration}[!ht] 
    \centering      
...
\textit{\en{Custom Javascript formatter}} και συμπληρώνουμε τον κώδικα που ακολουθεί παρακάτω. 
Στο τέλος πατάμε \textbf{\textit{\en{Save changes}}}.


\begin{Illustration}[!ht] 
  \centering      
  \includegraphics[width=1\textwidth]{figures/Payload_formatter_selection.png}       
  \caption{Κωδικοποίηση του \en{Default uplink payload formatter}.}
  \label{figure5.25}    
\end{Illustration} 

\subsubsection{Κώδικας \en{payload formatter}}
...
επικοινωνία με τον τριφασικό μετρητή, μπορούμε να ξεκινήσουμε την δοκιμή του συστήματος. Για αρχή κατασκευάζουμε ένα απλό 
κύκλωμα με τρεις λαμπτήρες ώστε να προσομοιώσουμε μία τριφασική κατανάλωση (Εικόνα \ref{figure5.24}). Συγκεκριμένα, 
από ένα ρευματολήπτη (\en{plug}) απομονώνουμε τη φάση \textbf{\en{L}} (\en{Live}) και τον 
ουδέτερο \textbf{\en{N}} (\en{Neutral}). Στη συνέχεια διακλαδώνουμε την φάση σε τρεις κλάδους (με συνδέσμους \en{WAGO}), όπου κάθε κλάδος 
...
τόσο \textbf{οι καταναλώσεις των λαμπτήρων} όσο και οι ίδιες \textbf{οι απώλειες/καταναλώσεις των \en{PZEM}}, παρέχοντας 
...
\begin{Illustration}[!ht] 
  ...
  \caption{Δοκιμαστικό κύκλωμα τριφασικού μετρητή με τρεις λαμπτήρες ως φορτία (φάσεις \en{L1-L3}). Το καλώδιο φάσης του ρευματολήπτη (μάυρο) χωρίζεται σε 3 καλώδια (κόκκινο, καφέ, μαύρο), τα οποία συνδέονται με ένα \en{PZEM} και ένα λαμπτήρα, αντιπροσωπεύοντας μία «υπο-φάση» το καθένα. Το καλώδιο ουδετέρου του ρευματολήπτη (μπλε) συνδέεται σε όλα τα \en{PZEM} και τους λαμπτήρες.}
  \label{figure5.26}    
\end{Illustration} 

\newpage

\noindent\textbf{Συνδεσμολογία δοκιμής (\en{PZEM}-λαμπτήρας):}
\begin{itemize}
  \item \textbf{\en{PZEM 1}} (κλάδος φάσης, κόκκινο καλώδιο): \en{OSRAM Bellalux} \textbf{75\,W}/\en{230\,V} (λευκός λαμπτήρας πυρακτώσεως). Ακροδέκτης \en{L} του \en{PZEM} στον αντίστοιχο κλάδο φάσης, ακροδέκτης \en{N} στον κοινό ουδέτερο. Ο \en{split CT} τυλίγει τον ίδιο αγωγό \en{L} με φορά «\en{source} $\rightarrow$ \en{load}».
  \item \textbf{\en{PZEM 2}} (κλάδος φάσης, καφέ καλώδιο): \en{Tungsram R63} \textbf{60\,W}/\en{240\,V} (ανακλαστήρας \en{spot}). Σύνδεση \en{L}/\en{N} όπως ανωτέρω, \en{CT} στον καφέ αγωγό προς το φορτίο.
  \item \textbf{\en{PZEM 3}} (κλάδος φάσης, μαύρο καλώδιο): \en{Philips} \textbf{60\,W}/\en{230\,V} (μπλε λαμπτήρας πυρακτώσεως). Σύνδεση \en{L}/\en{N} όπως ανωτέρω, \en{CT} στον μαύρο αγωγό προς το φορτίο.
\end{itemize}

\noindent\textit{Σημείωση καλωδίωσης:} Και τα τρία \en{PZEM-004T v3.0} τροφοδοτούνται/μετρούν από τους ακροδέκτες \en{L}/\en{N} (ο ουδέτερος είναι κοινός), ενώ ο μετασχηματιστής ρεύματος κάθε \en{PZEM} τοποθετείται μόνο στον αντίστοιχο αγωγό φάσης. Η φορά του \en{CT} πρέπει να είναι σταθερά από την πλευρά τροφοδοσίας προς το φορτίο, ώστε το ρεύμα να μετριέται με σωστό πρόσημο. Τα ονομαστικά \en{230/240\,V} των λαμπτήρων εξηγούν μικρές διαφορές στην αποδιδόμενη ισχύ, δεδομένου ότι για ωμικά φορτία ισχύει κατά προσέγγιση $P \propto V^{2}$.


Στη συνέχεια κάνουμε \en{upload} (βλ. Εικόνα \ref{figure5.17}) το \en{Arduino sketch} της Υπο-υποενότητας \ref{subsubsec:5.2.2.2} στο 
\en{The Things Uno} και συνδέουμε τον ρευματολήπτη στην πρήζα (230$V$), με προσοχή στις πολικότητες. Έπειτα κοιτάμε το \en{Serial monitor} και βλέπουμε:

\begin{Illustration}[!ht] 
  \centering      
  \includegraphics[width=0.9\textwidth]{figures/TEST_SM_OUTPUT_1.png}       
  \caption{Ροή εντολών \en{MAC} και επιτυχές \en{OTAA join} της συσκευής στο \en{EU868} (ρυθμίσεις καναλιών/ρυθμών, \en{DevAddr}, \en{DR5}, αποτυχημένη αλλαγή κλάσης ως αναμενόμενο).}
  \label{figure5.27}    
\end{Illustration} 

Στο στιγμιότυπο του \en{Serial Monitor} παραπάνω φαίνεται η πλήρης ακολουθία ρύθμισης και ένταξης του 
\en{The Things Uno} στο \en{LoRaWAN} δίκτυο. Πρώτα στέλνονται \textbf{εντολές \en{MAC}} που 
ορίζουν τις παραμέτρους για \textbf{\en{EU868}}: \textbf{\en{rx2 3 869525000}} (κανάλι \en{RX2} στα 
869.525$MHz$, \en{DR3}), εύρη δεδομένων (\en{drrange}) και \en{dcycle} για τα κανάλια 1-7, καθώς και οι 
συχνότητες 867.1-867.9$MHz$. Κατόπιν τίθενται \textbf{\en{pwridx 1}} (δείκτης ισχύος εκπομπής), 
\textbf{\en{retx 7}} (μέγιστες επαναμεταδόσεις) και \textbf{\en{dr 5}} (ρυθμός δεδομένων \textbf{\en{DR5}}, 
δηλ. \textit{\en{SF7BW125}} στο \en{EU868}). Ακολουθεί \textbf{\en{OTAA join}}: 
«\en{Join accepted. Status: 00000401}» και εκχώρηση \textbf{\en{DevAddr}} (\en{00DA247E}). Η εντολή 
«\textbf{\en{mac set class a}}» επιστρέφει \textbf{\en{invalid\_param}} επειδή τα \en{RN2483}/\en{RN2903} 
είναι \textit{εγγενώς \en{Class A}} και επομένως δεν απαιτούν/δέχονται αλλαγή κλάσης μέσω αυτής της εντολής. Το μπλοκ 
«\textbf{\en{-- STATUS}}» εμφανίζει \en{EUI}/\en{DevEUI}/\en{AppEUI}, τον ενεργό \textbf{\en{Data Rate: 5}}, 
και τις καθυστερήσεις \textbf{\en{RX1/RX2}} (5000/6000$ms$) που θα ισχύουν για τα \en{downlinks}. 
Μετά την επιτυχή ένταξη, ο κώδικας στέλνει περιοδικά \en{uplink} με το πακετάρισμα των 
μετρήσεων από τα τρία \en{PZEM}:

\begin{Illustration}[!ht] 
  \centering      
  \includegraphics[width=1\textwidth]{figures/TEST_SM_OUTPUT_2.png}       
  \caption{Ένδειξη \en{Serial Monitor} με μετρήσεις από 3 \en{PZEM-004T} και αποστολή \en{uplink} (\en{mac tx uncnf}) στο \en{TTN}.}
  \label{figure5.28}    
\end{Illustration} 

Στην Εικόνα \ref{figure5.28} φαίνονται οι πλήρεις μετρήσεις ανά φάση από τους τρεις αισθητήρες \en{PZEM} 
(\textit{τάση, ρεύμα, ισχύς, ενέργεια, συχνότητα, συντελεστής ισχύος}) πριν από κάθε αποστολή. 
Για παράδειγμα, ο \textbf{\en{PZEM 1}} μετρά 232.50$V$, 0.335$A$ και 77.20$W$, ενώ 
ο \textbf{\en{PZEM 2}} και \textbf{\en{PZEM 3}} δίνουν 53.30$W$ και 58.70$W$ αντίστοιχα. 
Αμέσως μετά, ο κώδικας πακετάρει τις τιμές 
(σύνολο \textbf{42 \en{bytes}}: \en{14 bytes/phase}, \en{big-endian}) και στέλνει το 
\textbf{\en{uplink}} με την εντολή \textbf{\en{mac tx uncnf 1 <hex payload>}}, η οποία εμφανίζεται 
στο τέλος ως δεκαεξαδική ακολουθία. 

Οι ενδείξεις της Εικόνας \ref{figure5.28} είναι πλήρως συνεπείς με τη φυσική των ωμικών φορτίων 
(\en{incandescent lamps}): ο συντελεστής ισχύος παραμένει ($\approx 1$) (\en{PF}=0.98-0.99), η συχνότητα 
σταθερή κοντά στα (50$Hz$), ενώ η ενεργός ισχύς προσεγγίζει άριστα τον θεωρητικό υπολογισμό ($P\simeq V\cdot I$) 
για κάθε κλάδο. Για τον \en{OSRAM 75 $W$/230 $V$} (\en{PZEM 1}) η μετρούμενη τιμή (77.2$W$) εξηγείται από την 
ελαφρώς αυξημένη τάση τροφοδοσίας (($\sim232.5V$)) και τις ονομαστικές ανοχές του λαμπτήρα. Για τον 
\en{Tungsram R63 60$W$/240$V$} (\en{PZEM 2}) η χαμηλότερη τάση (($\sim232.4V$)) οδηγεί σε ισχύ μικρότερη 
της ονομαστικής, όπως αναμένεται από τον κανόνα ($P\propto V^{2}$) (θεωρητικά ($\sim56W$), μετρημένα 
(53.3$W$)). Ο \en{Philips 60 W/230 V} (\en{PZEM 3}) αποδίδει (58.7$W$), εντός φυσιολογικών ανοχών 
και με μικρή επίδραση της θερμοκρασίας του νήματος. Τέλος, οι σωρευτικές τιμές \en{energy} (0.001-0.002$kWh$) 
(δηλ. (1-2$Wh$)) αντιστοιχούν σε λειτουργία λίγων λεπτών, επιβεβαιώνοντας ότι τόσο η κωδικοποίηση/αποστολή 
του \en{uplink} όσο και η μέτρηση των τριών \en{PZEM} λειτουργούν ορθά και επαναλήψιμα στο παρόν στήσιμο.



\noindent\textbf{Παραλαβή και αποκωδικοποίηση στο \en{The Things Stack}}

Με το που εκπέμψει το \en{TTU} το \en{uplink}, λαμβάνεται από το \en{Basic Station}, το οποίο το προωθεί 
(\en{forward}) μέσω \en{LNS} προς τον \en{Gateway Server} του
\en{The Things Stack}. Εκεί, το μήνυμα ελέγχεται, αποκρυπτογραφείται από τον \en{Network Server} και αποκωδικοποιείται σε φυσικές μονάδες (\en{decoded\_payload}).
Η όλη ροή αποτυπώνεται ζωντανά στην καρτέλα \en{Live data} της συσκευής (βλ. Εικόνα \ref{figure5.29}): πρώτα η αποδοχή ένταξης
(\en{Accept join-request}/\en{Successfully processed join-accept}), στη συνέχεια η προώθηση και επιτυχής
επεξεργασία των \en{uplinks} (\en{Forward uplink data message}/\en{Successfully processed data up message}),
μαζί με προεπισκόπηση του \en{FRMPayload} (δεκαεξαδικό) και του αποτελέσματος της αποκωδικοποίησης.

\begin{Illustration}[!ht]
  \centering
  \includegraphics[width=1.0\textwidth]{figures/TEST_TTS_CONNECT.png}
  \caption{Πίνακας \en{Live data} στο \en{TTS}: χρονολόγιο γεγονότων για \en{join-accept} και εισερχόμενα \en{uplinks}. Διακρίνονται τα πεδία \en{DevAddr}, η επισήμανση \en{Payload} με δεκαεξαδικά δεδομένα, καθώς και η προεπισκόπηση \en{decoded\_payload} όταν είναι ενεργός ο \en{formatter}.}
  \label{figure5.29}
\end{Illustration}

Στην Εικόνα \ref{figure5.29} φαίνεται αναλυτικά το χρονικό των μηνυμάτων. Δεξιά, στη στήλη \en{DATA PREVIEW},
διακρίνεται το \en{DevAddr} που αποδόθηκε κατά το \en{OTAA}, ενώ για κάθε \en{uplink} εμφανίζεται το ωφέλιμο
\en{FRMPayload} σε μορφή \en{hex}. Αν η λειτουργία \en{Verbose stream} είναι ενεργοποιημένη, προβάλλονται
επιπλέον πληροφορίες (π.χ.\ χρόνοι, τύποι γεγονότων, κ.ά.), χρήσιμες για έλεγχο και αποσφαλμάτωση. Τα κουμπιά
\en{Pause}/\en{Clear}/\en{Export as JSON} διευκολύνουν την παύση της ροής, την εκκαθάριση της λίστας και την
εξαγωγή των γεγονότων αντίστοιχα.

Ακολούθως, ο \en{Uplink Payload Formatter} μας εφαρμόζει τη σύμβαση
κωδικοποίησης που ορίσαμε στο \en{TTU} (κλίμακες τιμών, \en{big-endian} πακετάρισμα, \en{sentinel} για
\en{NaN}/\en{Inf}\\en{Neg}) και παράγει ένα δομημένο αντικείμενο \en{JSON} με τις τρεις φάσεις. Στην Εικόνα \ref{figure5.30}
φαίνονται οι μετρήσεις ανά αισθητήρα \en{PZEM} (\en{sensor1-sensor3}) σε φυσικές μονάδες, ακριβώς όπως θα τις
καταναλώσει λάβει η διαδικτυακή εφαρμογή που θα αναπτύξουμε στη συνέχεια (Κεφάλαιο \ref{chapter6}), μέσω \en{HTTP webhook}.

\begin{Illustration}[!ht]
  \centering
  \includegraphics[width=0.80\textwidth]{figures/TEST_DECODED_PAYLOAD.png}
  \caption{\en{decoded\_payload} από τον \en{formatter}: ανά \en{sensor} προβάλλονται \en{voltage}, \en{current},
  \en{power}, \en{energy}, \en{frequency}, \en{powerFactor}. Οι τιμές προκύπτουν από τις κλίμακες κωδικοποίησης
  του \en{uplink} και είναι έτοιμες για αποθήκευση/οπτικοποίηση χωρίς περαιτέρω μετασχηματισμούς.}
  \label{figure5.30}
\end{Illustration}

Παρατηρούμε ότι τα πεδία του \en{decoded\_payload} (τάση γύρω στα $232$-$233\,\mathrm{V}$, ρεύμα
$0.24$-$0.34\,\mathrm{A}$, ισχύς $49$-$77\,\mathrm{W}$, συχνότητα $\sim 50\,\mathrm{Hz}$, \en{PF} $0.98$-$0.99$,
ενέργεια $1$-$2\,\mathrm{Wh}$ για λίγα λεπτά λειτουργίας) ταυτίζονται με τις ενδείξεις του \en{Serial Monitor}
και επιβεβαιώνουν ότι η διαδρομή \en{Device} $\rightarrow$ \en{Basic Station} $\rightarrow$ \en{TTS} είναι
ορθή. Το γεγονός ότι η αποκωδικοποίηση γίνεται εντός του \en{TTS} σημαίνει ότι η downstream κατανάλωση δεδομένων
είναι απλούστερη: είτε μέσω \en{webhooks} (\en{JSON} \en{POST} στο \en{backend}) είτε μέσω \en{MQTT} οι ίδιες
τιμές διατίθενται ήδη σε φυσικές μονάδες, μειώνοντας την πολυπλοκότητα στην εφαρμογή. Επιπλέον, η προβολή
στο \en{Live data} επιτρέπει άμεσο εντοπισμό σφαλμάτων, όπως ασυνεπές \en{FPort}, μηδενικό \en{payload},
λανθασμένη κλίμακα ή αποτυχία του \en{formatter}, που είναι χρήσιμο να γνωρίζουμε πριν προχωρήσουμε στην 
ολοκλήρωση με το πληροφοριακό μας σύστημα.
