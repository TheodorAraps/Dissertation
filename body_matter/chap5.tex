...
void loop(§\cyan{)}§ {
  ...
  ttn.§\orange{sendBytes}§(data, sizeof(data§\cyan{))}§;
...
\begin{itemize}
  ...
  \item Δημιουργούμε τις σταθερές \textbf{\en{appEui}} και \textbf{\en{appKey}}, οι οποίες αποτελούν τα \en{OTAA credentials} της συσκευής. Η τιμή της \textbf{\en{appEUI}} προκύπτει από την διαδικασία που περιγράφηκε στην σημείωση στο τέλος της Υπο-υποενότητας ............, ενώ η τιμή της \textbf{\en{appKey}} προκύπτει από τις οδηγίες στην Υποενότητα ............\dots
...
\begin{enumerate}
  ...
  \item \textbf{Κωδικοποίηση \en{payload}:} Για κάθε \en{PZEM} πακετάρονται 6 τιμές με μέγεθος 2 \en{bytes} η κάθε μία, οπότε \textbf{12 \en{bytes}} ανά \en{PZEM} (\textbf{σύνολο 36 \en{bytes}}). Αποθηκεύονται ως \textbf{\en{16-bit}} τιμές \textbf{\en{big-endian}} (\en{high byte} πρώτα), με κλίμακες:
\begin{table}[ht]
\centering
...
\begin{tabular}{|l|c|c|}
\hline
\textbf{Μέγεθος} & \textbf{Κλίμακα} & \textbf{Εύρος τιμών} \\
\hline
Τάση & 
$\times 10 (0.1 V/LSB)$ &
$0 \ldots 6553.5 V$ \\
...
\hline
\end{tabular}
\caption{Κλίμακες κωδικοποίησης και εύρη τιμών ανά μέγεθος.}
\label{tab:enc_scales_ranges}
\end{table}

  \item \textbf{\en{Uplink}:} Στέλενται το μήνυμα \en{uplink} στο \en{TTS} με την μέθοδο \en{ttn.sendBytes(data, sizeof(data))}.
\end{enumerate}
