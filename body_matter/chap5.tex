...
\begin{itemize}
  ...
  \item Δημιουργούμε τις σταθερές \textbf{\en{appEui}} και \textbf{\en{appKey}}, οι οποίες αποτελούν τα \en{OTAA credentials} της συσκευής. Η τιμή της \textbf{\en{appEUI}} προκύπτει τρέχοντας το \en{sketch} που βρίσκεται στην διαδρομή \textit{\en{File > Examples > TheThingsNetwork > Deviceinfo}}. Αφού το επιλέξουμε, θέτουμε στην μεταβλητή \textbf{\en{freqPlan}} την τιμή \textit{\en{TTN\_FP\_EU868}} πατάμε το κουμπί του \en{Upload}:
  \begin{Illustration}[!ht]     
    \centering    
  ...
  % \item \en{\#define loraSerial Serial1}: Η \en{The Things Uno} (βασισμένη σε \en{ATmega32u4}/\en{Leonardo}) χρησιμοποιεί τη \en{Serial1} για σύνδεση με το \en{LoRa} \en{module} (π.χ. \en{RN2483}/\en{RN2903}), αφήνοντας την \en{Serial} για \en{debug} προς τον \en{USB}.
  % \item \en{\#define debugSerial Serial}: \en{USB CDC} για μηνύματα κατάστασης/εντοπισμό σφαλμάτων.
  % \item \en{\#define freqPlan TTN\_FP\_EU868}: Επιλογή \en{frequency plan} \en{EU868} (ταιριάζει με τη ρύθμιση της εφαρμογής στο \en{TTS}).
  % \item \en{TheThingsNetwork ttn(loraSerial, debugSerial, freqPlan)}: Δημιουργεί \en{TTN} \en{stack} αντικείμενο με \en{hardware UART} για \en{LoRa} και \en{USB} \en{UART} για \en{logs}.
\end{itemize}
...
%%%%   Υποενότητα 5.2.2: Δημιουργία Application στο The Things Stack   %%%%
